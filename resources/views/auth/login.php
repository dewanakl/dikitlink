...
<form method="POST" onsubmit="login()" id="submit-form">
    ...
    <div class="form-floating mb-3">
        <input type="email" name="email" class="form-control <?= error('email', 'is-invalid') ?>" id="floatingInputlogin" placeholder="Email" value="<?= old('email') ?>" autocomplete="on">
    ...
    <div class="form-floating mb-3">
        <input type="password" name="password" class="form-control <?= error('password', 'is-invalid') ?>" id="floatingPasswordlogin" placeholder="Password" autocomplete="on">
    ...
    <div class="d-grid">
        <button class="g-recaptcha btn btn-primary fw-bold shadow-sm my-2" id="button-login" type="submit" <?php if (env('CAPTCHA_WEB')) : ?> onclick="login()" data-sitekey="<?= env('CAPTCHA_WEB') ?>" data-callback="onSubmit" data-action="login" <?php endif ?>>Masuk</button>
    </div>
    <hr class="text-dark">
...
<script>
    const login = () => {
        ...
        btn.disabled = true;
        btn.className = 'g-recaptcha btn btn-primary active disabled fw-bold shadow-sm my-2'
